...
class NASTrain(Simulator):
    ...
    def nas_enhancement(self):
        enhanced_util = self.util_down_avg[self.action_down]
        enhance_level = -1
        ...
        if res_index < (len(VIDEO_BIT_RATES) - 1) and playback_deadline > 0:
            ...
            for i in range(num_enh_methds):
                ...
                enh_quality = self.util_enh_avg[res_index][i]
                if enh_delay < playback_deadline and enh_quality > best_enh_quality:
                    best_enh_quality = enh_quality
                    enhanced_util = best_enh_quality
        return enhanced_util, enhance_level
    ...
    def _act(self):
        ...
        enhanced_util, enhance_level = self.nas_enhancement()
        self.idx_down += 1
    ...
    def control_download_finish(self):
        ...
        if res_index < len(VIDEO_BIT_RATES) - 1 and playback_deadline > 0:
            ...
            for i in reversed(range(num_enh_methds)):
                enh_delay = enh_slice[i]
                enh_quality = self.vmaf_enh_seg[self.idx_down - 1][res_index][i]
                if enh_delay < playback_deadline and enh_quality > best_enh_quality:
                    enh_method = i
                    best_enh_quality = enh_quality
            if enh_method > -1:
