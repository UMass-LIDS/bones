...
RANDOM_SEED = 42
SUMMARY_DIR = '../rl/nas/results_logb_10k/'
LOG_FILE = '../rl/nas/results_logb_10k/log'
...
def agent(agent_id, net_params_queue, exp_queue, pensieveArgs):
    ...
    with open(LOG_FILE + '_agent_' + str(agent_id), 'w') as log_file:
        ...
        # net._actor.load_state_dict(torch.load("../rl/pensieve/actor.pt"))
        time_stamp = 0
...
class NASTrain(Simulator):
    def __init__(self, args):
        ...
        self.util_down_avg, self.util_enh_avg = self.calculate_util_bitrate()
        # self.util_down_avg, self.util_enh_avg = self.calculate_util_vmaf()
        ...
        for idx_bitrate in range(self.enhance_time.shape[0]):
            ...
            for idx_action in range(self.enhance_time.shape[1]):
                if self.enhance_time[idx_bitrate][idx_action] < self.seg_time:
                    if self.util_enh_avg[idx_bitrate][idx_action] > best_quality:
                        best_quality = self.util_enh_avg[idx_bitrate][idx_action]
    ...
    def act(self, idx_action):
        ...
        rebuffer, enhanced_util = self._act()
    ...
    def nas_enhancement(self):
        ...
        enhanced_util = self.util_enh_avg[action_down][self.action_enh]
        self.idx_enh += 1
        return enhanced_util
    ...
    def _act(self):
        ...
        self.buffer_enhance.consume(time_download)
        ...
        enhanced_util = self.nas_enhancement()
        self.idx_down += 1
        ...
        self.buffer_enhance.consume(time_wait)
        ...
        return rebuffer, enhanced_util

    def calculate_util_bitrate(self):
        ssim_down_avg = np.mean(self.ssim_down_seg, axis=0)
        ssim_enh_avg = np.mean(self.ssim_enh_seg, axis=0)

        # linear interpolation
        effective_bitrate = np.ones((ssim_enh_avg.shape))
        for i in range(ssim_enh_avg.shape[0] - 1):
            for j in range(1, ssim_enh_avg.shape[1]):
                ssim = ssim_enh_avg[i, j]
                for k in range(self.bitrate.shape[0] - 1):
                    if ssim_down_avg[k] <= ssim < ssim_down_avg[k + 1]:
                        effective_bitrate[i, j] = self.bitrate[k] + (self.bitrate[k + 1] - self.bitrate[k]) * (
                                    ssim - ssim_down_avg[k]) / (ssim_down_avg[k + 1] - ssim_down_avg[k])
                        break

        util_enh_avg = np.log(effective_bitrate / self.bitrate[0])
        util_enh_avg[:, 0] = np.log(self.bitrate / self.bitrate[0])
        util_enh_avg[-1, 1:] = -np.inf

        util_down_avg = np.log(self.bitrate / self.bitrate[0])
        return util_down_avg, util_enh_avg

    def calculate_util_vmaf(self):
        # util_down_avg = np.mean(self.vmaf_down_seg, axis=0)
        # util_enh_avg = np.mean(self.vmaf_enh_seg, axis=0)
        util_down_avg = self.vmaf_down_avg
        util_enh_avg = self.vmaf_enh_avg
        return util_down_avg, util_enh_avg
