...
@ModuleOption("enhance_buffer", requires=[Enhancer], default=True)
class EnhanceBufferImpl(Module, BufferManager):
    """
    ...
    def __init__(self) -> None:
        super().__init__()
        self.log = logging.getLogger("EnhanceBuffer")
    ...
    async def setup(self, config: PlayerConfig, enhancer: Enhancer, **kwargs):
    ...
    async def dequeue(self) -> Tuple[int, Dict[int, Segment]]:
        async with self._accessible:
            ...
            while self._indices.empty():
                await self._accessible.wait()
                if self._is_end:
                    self.log.info("Enhance buffer closed.")
                    return None, None
    ...
    def buffer_level(self, continuous: bool = False) -> float:
        if continuous:
            return self._buffer_level + self.enhancer.remain_task()
