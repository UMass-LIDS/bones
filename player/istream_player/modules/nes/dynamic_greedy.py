...
@ModuleOption("dynamic_greedy", requires=[DownloadBufferImpl, EnhanceBufferImpl, Enhancer, Scheduler, BandwidthMeter], default=True)
class DynamicGreedyController(Module, NESController):
    log = logging.getLogger("DynamicGreedyController")
    def __init__(self):
        super().__init__()
        self.switch = 10000  # switch threshold
        self.use_tput = True
    ...
    def update_selection(self, adaptation_sets: Dict[int, AdaptationSet], index: int) -> Tuple[Dict[int, int], Dict[int, int]]:
        ...
        for adaptation_set in adaptation_sets.values():
            if self.first_download:
                ...
                download_action, enhance_action = self.dynamic_greedy_decision(adaptation_set)
    ...
    def dynamic_greedy_decision(self, adaptation_set: AdaptationSet):
        ...
        if self.use_tput and self.buff_down >= self.switch and quality_bones >= quality_tput:
            self.use_tput = False
        if (not self.use_tput) and self.buff_down < self.switch and quality_bones < quality_tput:
            self.use_tput = True

        if self.use_tput:
            action_down = act_tput
