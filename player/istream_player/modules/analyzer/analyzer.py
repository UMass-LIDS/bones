...
@ModuleOption("data_collector", default=True, requires=[MPDProvider, BandwidthMeter, Scheduler, Player, DownloadBufferImpl, EnhanceBufferImpl])
class PlaybackAnalyzer(
    ...
    async def setup(
        ...
        **kwargs,
        ...
        download_buffer.add_listener(self)
        enhance_buffer.add_listener(self)
    ...
    async def on_segment_playback_start(self, segments: Dict[int, Segment]):
        for segment in segments.values():
            self._segments_by_url[segment.url].download_action = segment.download_action
            if segment.decode_data is not None:  # complete enhancement
                self._segments_by_url[segment.url].enhance_action = segment.enhance_action
            else:
                self._segments_by_url[segment.url].enhance_action = 0
    ...
    def save(self, output: io.TextIOBase | TextIO) -> None:
        ...
        headers = ("Index", "Start", "End", "Download", "Enhance", "Bitrate", "Adap-Th", "Seg-Th", "Ratio", "URL")
        output.write("%-10s%-10s%-10s%-10s%-10s%-10s%-10s%-10s%-10s%-20s\n" % headers)
        for segment in sorted(self._segments_by_url.values(), key=lambda s: s.index):
            if last_quality is None:
                ...
                if last_quality != segment.quality:
                    ...
                    quality_switches += 1
            output.write(
                "%-10d%-10.1f%-10.1f%-10d%-10d%-10d%-10d%-10d%-10.1f%-20s\n"
                % (
                    segment.index,
                    self._seconds_since(segment.start_time or 0),
                    self._seconds_since(segment.stop_time or 0),
                    segment.download_action,
                    segment.enhance_action,
                    segment.bitrate,
        ...
        for time, state, position in self._states:
            if state == State.BUFFERING and buffering_start is None:
                ...
                output.write("%-10.2f%-10.2f%-10.2f\n" % (buffering_start, time, duration))
